...
pub fn parse_geojp2_payload(data: &[u8]) -> Result<CrsInfo> {
    ...
    let le = match &data[0..2] {
        ...
        b"MM" => false,
        _ => {
            return Err(Jp2Error::InvalidGeoMetadata(
                "unknown byte order in mini-TIFF".into(),
            ))
        }
    };
        ...
        match (tag, dtype) {
            (t, 12)
                if t == gtag::MODEL_PIXEL_SCALE
                    || t == gtag::MODEL_TIEPOINT
                    || t == gtag::MODEL_TRANSFORMATION =>
            {
...
pub fn parse_gmljp2_xml_payload(xml: &str) -> Option<CrsInfo> {
    ...
    fn parse_epsg(s: &str) -> Option<u16> {
        let digits_rev: String = s.chars().rev().take_while(|c| c.is_ascii_digit()).collect();
    ...
    let origin_pos =
        extract_first(xml, "<gml:pos>", "</gml:pos>").and_then(|s| parse_two_f64(&s))?;
    ...
    let srs_name = extract_first(xml, "srsName=\"", "\"").or_else(|| {
...
mod tests {
    ...
    fn roundtrip_geo_meta() {
        ...
        assert!(
            (parsed_gt.x_origin - 10.0).abs() < 1e-9,
            "x_origin mismatch"
        );
        assert!(
            (parsed_gt.y_origin - 49.0).abs() < 1e-9,
            "y_origin mismatch"
        );
        assert!(
            (parsed_gt.pixel_width - 0.001).abs() < 1e-9,
            "pixel_width mismatch"
        );
        assert!(
            (parsed_gt.pixel_height - (-0.001)).abs() < 1e-9,
            "pixel_height mismatch"
        );
