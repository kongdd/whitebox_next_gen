...
use super::boxes::{
    box_type, write_box, write_file_type, write_signature, write_super_box, write_uuid_box,
    write_xml_box, ColourSpec, ImageHeader, ResolutionBox, GEOJP2_UUID,
};
...
impl GeoJp2Writer {
    ...
    pub fn compression(mut self, c: CompressionMode) -> Self {
        self.compression = c;
        self
    }
    /// Number of DWT decomposition levels (0–15; default auto-capped by image size).
    pub fn decomp_levels(mut self, n: u8) -> Self {
        self.decomp_levels = n.min(15);
        self
    }
    /// Manually set bit depth (default inferred from data type).
    pub fn bits_per_sample(mut self, b: u8) -> Self {
        self.bits = b;
        self
    }
    ...
    pub fn xml_metadata(mut self, xml: impl Into<String>) -> Self {
        self.xml_metadata = Some(xml.into());
        self
    }
    ...
    fn write_raw<W: Write + Seek>(&self, mut w: W, pixels: &[i32]) -> Result<()> {
        ...
        if self.geo_transform.is_some() || self.epsg.is_some() {
            let model_type = if self.epsg.map_or(false, |e| e / 1000 == 4) {
                2u16
            } else {
                1
            };
    ...
    fn encode_codestream(&self, pixels: &[i32]) -> Result<Vec<u8>> {
        ...
        let cod = if lossless {
            Cod::lossless(nl, self.components)
        } else {
            Cod::lossy(nl, self.components)
        };
        let qcd = if lossless {
...
mod writer_tests {
    ...
    fn writer_lossless_u16_single_band_parse_and_decode_shape_smoke() {
        ...
        let read_back = jp2
            .read_band_u16(0)
            .expect("reader should decode single-band data");
        assert_eq!(read_back.len(), data.len(), "decoded band length mismatch");
        assert!(
            read_back.iter().any(|&v| v != 0),
            "decoded band should not be fully zero"
        );
    }
    ...
    fn writer_multiband_decode_roundtrip_smoke() {
        ...
        assert_ne!(
            band0, band1,
            "decoded channels should not collapse to identical data"
        );
        assert_ne!(
            band1, band2,
            "decoded channels should not collapse to identical data"
    ...
    fn writer_lossy_mode_roundtrip_metadata_smoke() {
        ...
        assert!(
            !jp2.is_lossless(),
            "lossy writer output should not be flagged lossless"
        );
    }
    ...
    fn writer_geo_metadata_roundtrip_preserves_transform_epsg_and_nodata() {
        ...
        let jp2 =
            GeoJp2::from_bytes(&cur.into_inner()).expect("reader should parse georeferenced JP2");
    ...
    fn a4_u8_multiband_level_shift_produces_unsigned_output_range() {
        ...
        let data_u16: Vec<u16> = (0..(w * h * nc as u32)).map(|i| (i % 256) as u16).collect();
        ...
        assert_eq!(
            jp2.bits_per_sample(),
            16,
            "A4: bits_per_sample should be 16"
        );
        ...
        for band in 0..nc as usize {
            ...
            assert_eq!(
                decoded.len(),
                (w * h) as usize,
                "A4: band {} length mismatch",
                band
            );
            // Determinism check: repeated decode of same band must be identical.
            let decoded_again = jp2
                .read_band_u16(band)
                .expect("A4: repeated band decode should succeed");
            assert_eq!(
                decoded, decoded_again,
                "A4: band {} decode is not deterministic across repeated reads",
                band
            );
        }
    ...
    fn a4_u16_multiband_decode_bands_are_distinct_and_in_range() {
        ...
        let jp2 =
            GeoJp2::from_bytes(&cur.into_inner()).expect("A4: reader should parse multiband JP2");
        ...
        assert_eq!(
            b0,
            jp2.read_band_u16(0).expect("A4: repeated band 0 decode")
        );
        assert_eq!(
            b1,
            jp2.read_band_u16(1).expect("A4: repeated band 1 decode")
        );
        assert_eq!(
            b2,
            jp2.read_band_u16(2).expect("A4: repeated band 2 decode")
        );
        assert_eq!(
            b0_i16,
            jp2.read_band_i16(0)
                .expect("A4: repeated band 0 i16 decode")
        );
