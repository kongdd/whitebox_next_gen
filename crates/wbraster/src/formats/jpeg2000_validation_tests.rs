...
mod jpeg2000_byte_alignment_validation {
    ...
    const RGB_8X8_FIXTURE: &[u8] = include_bytes!("../../tests/fixtures/rgb_8x8_lossless.jp2");
    const SENTINEL_STYLE_16X16_4B_FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/sentinel_style_16x16_4band_lossless.jp2");
    const TILED_RGB_64X64_BLOCK32_FIXTURE: &[u8] =
        include_bytes!("../../tests/fixtures/tiled_rgb_64x64_block32_lossless.jp2");
    ...
    fn a5_sentinel_style_fixture_metadata_and_probes() {
        let jp2 = crate::formats::jpeg2000_core::reader::GeoJp2::from_bytes(
            SENTINEL_STYLE_16X16_4B_FIXTURE,
        )
        ...
        for (band, row, col, expected) in probes {
            ...
            assert_eq!(
                got, expected,
                "A5 sentinel probe mismatch at band={band}, row={row}, col={col}"
            );
        }
    }
        ...
        for band in 0isize..4isize {
            for row in 0isize..16isize {
                for col in 0isize..16isize {
                    ...
                    assert_eq!(
                        got, expected,
                        "A4 sentinel formula mismatch at band={band}, row={row}, col={col}"
                    );
    ...
    fn a5_tiled_multicomponent_fixture_metadata_and_probes() {
        let jp2 = crate::formats::jpeg2000_core::reader::GeoJp2::from_bytes(
            TILED_RGB_64X64_BLOCK32_FIXTURE,
        )
        ...
        for (band, row, col, expected) in probes {
            ...
            assert_eq!(
                got, expected,
                "A5 tiled probe mismatch at band={band}, row={row}, col={col}"
            );
        }
    }
    ...
    fn test_b03_pixel_values_match_reference() {
        ...
        let reference_pixels: Vec<u16> = vec![
            3032, 3056, 3258, 3216, 3070, 3140, 3220, 3376, 3448, 3500, 3460, 3524, 3536, 3588,
            3708, 3520, 3170, 3112, 2974, 3094, 3144, 3132, 3022, 2876, 2990, 3034, 3012, 3140,
            3066, 3036, 3184, 3238, 3148, 3360, 3360, 3412, 3758, 3788, 3654, 3612, 3418, 3462,
            3454, 3340, 3162, 3212, 3236, 3204, 3340, 3478, 3352, 3468, 3676, 3610, 3294, 3230,
            3392, 3248, 3396, 3540, 3508, 3456, 3448, 3476, 3612, 3476, 3472, 3204, 3230, 3464,
            3528, 4092, 5428, 5988, 5604, 4852, 4040, 3462, 3296, 3272, 3264, 3272, 3268, 3226,
            3364, 3722, 3500, 3506, 3368, 3322, 3354, 3412, 3328, 3204, 3210, 3230, 3278, 3240,
            3216, 3242, 3266, 3240, 3240, 3322, 3282, 3332, 3584, 3472, 3500, 3452, 3104, 3136,
            3278, 3230, 3186, 3258, 3038, 3072, 3172, 3140, 3180, 3230, 3296, 3286, 3408, 3418,
            3618, 4204, 4088, 4424, 4472, 4428, 4244, 3644, 3330, 3212, 3196, 3136, 3152, 3308,
            3330, 3158, 3166, 3616, 4060, 4136, 3972, 4732, 4816, 3432, 3374, 3296, 3268, 3228,
            3324, 3224, 3160, 3264, 3164, 3436, 3380, 3224, 3196, 3128, 3180, 3128, 3172, 3184,
            3124, 3116, 3086, 3056, 2968, 2860, 2876, 2896, 3072, 3136, 3156, 3162, 3078, 2988,
            3180, 3232, 3196, 3138, 3196, 3094, 3130, 3284, 3236, 3288, 3256, 3204, 3178, 3094,
            3190, 3160, 3118, 3180, 4324, 4576, 3816, 4208, 3894, 3826, 5064, 4836, 3782, 3268,
            3078, 3284, 3368, 3480, 3476, 3410, 3418, 3430, 3552, 3500, 4400, 3512, 3376, 3354,
            3318, 3344, 3356, 3392, 3376, 3316, 3206, 3168, 3166, 3088, 3092, 3070, 3022, 3056,
            3002, 3148, 3080, 3080, 3064, 2992, 3008, 3012, 3028, 3000, 2938, 2940, 3074, 3058,
            3024, 3100, 3102, 3034,
        ];
        ...
        match read(b03_path) {
            Ok(raster) => {
                ...
                if native_pixels.len() < 256 {
                    eprintln!(
                        "WARNING: Only {} pixels available, expected 256",
                        native_pixels.len()
                    );
                }
                ...
                for (i, (&native, &reference)) in native_pixels
                    .iter()
                    ...
                    .enumerate()
                {
                    ...
                    if error > 50 {
                        // Allow tolerance for rounding/bit-depth differences
                        if mismatches < 10 {
                            eprintln!(
                                "[b03_validation] Pixel[{}]: native={} reference={} error={}",
                                i, native, reference, error
                            );
                        }
                        mismatches += 1;
                ...
                eprintln!(
                    "[b03_validation] Total mismatches (>50 error): {}/{}",
                    mismatches,
                    native_pixels.len()
                );
    ...
    fn test_ll_codeblock_0_0_isolation() {
        ...
        match read(b03_path) {
            Ok(raster) => {
                eprintln!(
                    "[ll_cb_isolation] Raster decoded successfully: {}x{}",
