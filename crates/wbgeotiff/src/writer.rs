...
impl GeoTiffWriter {
    ...
    pub fn layout(mut self, l: WriteLayout) -> Self {
        self.layout = l;
        self
    }
    ...
    pub fn software(mut self, s: impl Into<String>) -> Self {
    ...
    pub fn rows_per_strip(mut self, rps: u32) -> Self {
        self.layout = WriteLayout::Stripped {
            rows_per_strip: rps.max(1),
        };
        self
    }
    ...
    fn validate(&self, n: usize) -> Result<()> {
        ...
        if n != expected {
            Err(GeoTiffError::DataSizeMismatch {
                expected,
                actual: n,
            })
        } else {
    ...
    pub(crate) fn write_raw<W: Write + Seek>(&self, mut w: W, pixel_bytes: &[u8]) -> Result<()> {
        ...
        let tags = self.build_tags(bps, spp, &chunk_layout, &geo_keys_encoded);
    ...
    fn build_tags(
        ...
        push_short(
            ...
            self.planar_config.tag_value() as u32,
        );
        push_short(
            &mut tags,
            tag::SampleFormat,
    ...
    fn write_classic<W: Write + Seek>(
        ...
        for _ in 0..3 {
            ...
            let chunk_offsets: Vec<u64> = chunk_data
                .iter()
                .map(|c| {
                    let o = off;
                    off += c.len() as u64;
            ...
            for t in tags.iter_mut() {
                if t.code == tag::StripOffsets || t.code == tag::TileOffsets {
                    t.extra_data = chunk_offsets
                        .iter()
                        .flat_map(|&v| BO.u32_bytes(v as u32))
                        .collect();
        ...
        w.write_all(&BO.u32_bytes(ifd_offset))
            .map_err(GeoTiffError::Io)?;

        // Write IFD
        w.write_all(&BO.u16_bytes(tags.len() as u16))
            .map_err(GeoTiffError::Io)?;
        for t in &tags {
            w.write_all(&BO.u16_bytes(t.code))
                .map_err(GeoTiffError::Io)?;
            w.write_all(&BO.u16_bytes(t.data_type))
                .map_err(GeoTiffError::Io)?;
            w.write_all(&BO.u32_bytes(t.count))
                .map_err(GeoTiffError::Io)?;
            if t.extra_data.len() <= 4 {
                let mut b = [0u8; 4];
                ...
                w.write_all(&BO.u32_bytes(t.extra_offset64 as u32))
                    .map_err(GeoTiffError::Io)?;
            }
        }
        ...
        for t in &tags {
            if t.extra_data.len() > 4 {
                ...
                if t.extra_data.len() % 2 != 0 {
                    w.write_all(&[0u8]).map_err(GeoTiffError::Io)?;
                }
            }
        }
    ...
    fn write_bigtiff<W: Write + Seek>(
        ...
        w.write_all(&BO.u64_bytes(ifd_offset))
            .map_err(GeoTiffError::Io)?;

        // Write IFD
        w.write_all(&BO.u64_bytes(tags.len() as u64))
            .map_err(GeoTiffError::Io)?;
        for t in &tags {
            w.write_all(&BO.u16_bytes(t.code))
                .map_err(GeoTiffError::Io)?;
            w.write_all(&BO.u16_bytes(t.data_type))
                .map_err(GeoTiffError::Io)?;
            w.write_all(&BO.u64_bytes(t.count as u64))
                .map_err(GeoTiffError::Io)?;
            if t.extra_data.len() <= 8 {
                let mut b = [0u8; 8];
                b[..t.extra_data.len()].copy_from_slice(&t.extra_data);
                ...
                w.write_all(&BO.u64_bytes(t.extra_offset64))
                    .map_err(GeoTiffError::Io)?;
            }
        }
...
pub(crate) fn push_short(tags: &mut Vec<TiffTag>, code: u16, v: u32) {
    tags.push(TiffTag {
        ...
        data_type: 3,
        count: 1,
        extra_data: BO.u16_bytes(v as u16).to_vec(),
        extra_offset64: 0,
    });
}
...
pub(crate) fn push_long(tags: &mut Vec<TiffTag>, code: u16, v: u32) {
    tags.push(TiffTag {
        ...
        data_type: 4,
        count: 1,
        extra_data: BO.u32_bytes(v).to_vec(),
        extra_offset64: 0,
    });
}
