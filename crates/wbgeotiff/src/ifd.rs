...
impl ByteOrder {
    pub(crate) fn read_u16(self, buf: &[u8; 2]) -> u16 {
        match self {
            Self::LittleEndian => u16::from_le_bytes(*buf),
            Self::BigEndian => u16::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_u32(self, buf: &[u8; 4]) -> u32 {
        match self {
            Self::LittleEndian => u32::from_le_bytes(*buf),
            Self::BigEndian => u32::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_u64(self, buf: &[u8; 8]) -> u64 {
        match self {
            Self::LittleEndian => u64::from_le_bytes(*buf),
            Self::BigEndian => u64::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_i16(self, buf: &[u8; 2]) -> i16 {
        match self {
            Self::LittleEndian => i16::from_le_bytes(*buf),
            Self::BigEndian => i16::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_i32(self, buf: &[u8; 4]) -> i32 {
        match self {
            Self::LittleEndian => i32::from_le_bytes(*buf),
            Self::BigEndian => i32::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_i64(self, buf: &[u8; 8]) -> i64 {
        match self {
            Self::LittleEndian => i64::from_le_bytes(*buf),
            Self::BigEndian => i64::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_f32(self, buf: &[u8; 4]) -> f32 {
        match self {
            Self::LittleEndian => f32::from_le_bytes(*buf),
            Self::BigEndian => f32::from_be_bytes(*buf),
        }
    }
    pub(crate) fn read_f64(self, buf: &[u8; 8]) -> f64 {
        match self {
            Self::LittleEndian => f64::from_le_bytes(*buf),
            Self::BigEndian => f64::from_be_bytes(*buf),
        }
    }
...
impl IfdValue {
    ...
    pub fn as_bytes(&self) -> Option<&[u8]> {
        match self {
            Self::Bytes(v) | Self::Undefined(v) => Some(v.as_slice()),
            _ => None,
        }
    }
...
impl<R: Read + Seek> TiffReader<R> {
    ...
    pub fn new(mut reader: R) -> Result<Self> {
        ...
        let (variant, first_ifd_offset) = match magic {
            ...
            43 => {
                ...
                if offset_size != 8 {
                    return Err(GeoTiffError::InvalidTiff(format!(
                        "BigTIFF: expected offset size 8, got {}",
                        offset_size
                    )));
                }
    ...
    fn read_offset(&mut self) -> std::io::Result<u64> {
        match self.variant {
            TiffVariant::Classic => self.read_u32().map(|v| v as u64),
            TiffVariant::BigTiff => self.read_u64(),
        }
    }
    ...
    fn decode_value(
        ...
        let data: Vec<u8> = if total_bytes <= inline_size {
            ...
            let offset: u64 = match self.variant {
                TiffVariant::Classic => {
                    self.byte_order
                        .read_u32(inline_buf[..4].try_into().unwrap()) as u64
                }
                TiffVariant::BigTiff => self
                    .byte_order
                    .read_u64(inline_buf[..8].try_into().unwrap()),
            };
        ...
        Ok(match data_type {
            ...
            DataType::Ascii => IfdValue::Ascii(
                String::from_utf8_lossy(&data)
                    .trim_end_matches('\0')
                    .to_owned(),
            ),
            DataType::Short => IfdValue::Shorts(
                data.chunks_exact(2)
                    .map(|c| bo.read_u16(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::Long => IfdValue::Longs(
                data.chunks_exact(4)
                    .map(|c| bo.read_u32(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::Long8 => IfdValue::Long8s(
                data.chunks_exact(8)
                    .map(|c| bo.read_u64(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::Rational => IfdValue::Rationals(
                data.chunks_exact(8)
                    .map(|c| {
                        (
                            bo.read_u32(c[0..4].try_into().unwrap()),
                            bo.read_u32(c[4..8].try_into().unwrap()),
                        )
                    })
                    .collect(),
            ),
            ...
            DataType::SShort => IfdValue::SShorts(
                data.chunks_exact(2)
                    .map(|c| bo.read_i16(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::SLong => IfdValue::SLongs(
                data.chunks_exact(4)
                    .map(|c| bo.read_i32(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::SLong8 => IfdValue::SLong8s(
                data.chunks_exact(8)
                    .map(|c| bo.read_i64(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::SRational => IfdValue::SRationals(
                data.chunks_exact(8)
                    .map(|c| {
                        (
                            bo.read_i32(c[0..4].try_into().unwrap()),
                            bo.read_i32(c[4..8].try_into().unwrap()),
                        )
                    })
                    .collect(),
            ),
            DataType::Float => IfdValue::Floats(
                data.chunks_exact(4)
                    .map(|c| bo.read_f32(c.try_into().unwrap()))
                    .collect(),
            ),
            DataType::Double => IfdValue::Doubles(
                data.chunks_exact(8)
