...
impl FastAlmostGaussianFilterTool {
        ...
        match op {
            ...
            Phase3Op::Unsharp => {
                ...
                params.push(ToolParamSpec {
                    name: "amount",
                    description:
                        "Sharpening amount multiplier applied to residual (default 100.0).",
                    required: false,
                });
                params.push(ToolParamSpec {
                    name: "threshold",
            ...
            Phase3Op::Lee => {
                ...
                params.push(ToolParamSpec {
                    name: "sigma",
                    description:
                        "Intensity inclusion half-width around center value (default 10.0).",
                    required: false,
                });
                params.push(ToolParamSpec {
                    name: "m_value",
                    description:
                        "Minimum in-range sample count before fallback averaging (default 5.0).",
                    required: false,
                });
            }
            Phase3Op::RefinedLee => {
    ...
    fn parse_window_sizes(
        args: &ToolArgs,
        default_x: usize,
        default_y: usize,
    ) -> (usize, usize, isize, isize) {
        let sx = Self::normalize_odd_size(
            args.get("filter_size_x")
                .and_then(|v| v.as_u64())
                .unwrap_or(default_x as u64) as usize,
            3,
        );
        let sy = Self::normalize_odd_size(
            args.get("filter_size_y")
                .and_then(|v| v.as_u64())
                .unwrap_or(default_y as u64) as usize,
            3,
        );
        ...
        let output = match op {
                                ...
                                for c in 0..cols {
                                    ...
                                    for ny in (row - my)..=(row + my) {
                                        for nx in (col - mx)..=(col + mx) {
                                            ...
                                            let zn =
                                                if packed_rgb { value2i(zn_raw) } else { zn_raw };
                                            let zr = zn - min_val;
                                            sum += zr;
                                            sum2 += zr * zr;
                                            n += 1.0;
                                    ...
                                    if variance > 0.0 {
                                        ...
                                        out_row[c] = if ((z - mean) / s).abs() > threshold {
                                            mean
                                        } else {
                                            z
                                        };
            ...
            Phase3Op::Lee => {
                ...
                let sigma = args
                    ...
                    .unwrap_or(10.0)
                    .abs();
                let mut m_value = args.get("m_value").and_then(|v| v.as_f64()).unwrap_or(5.0);
            ...
            Phase3Op::ConservativeSmoothing => {
                ...
                let out_vals: Vec<Vec<f64>> = (0..bands)
                    ...
                    .map(|band_idx| {
                        ...
                        let fetch_value =
                            |rr: usize, cc: usize, band_buf: &Option<Vec<f64>>| -> f64 {
                                        ...
                                        for ny in (row - my)..=(row + my) {
                                            for nx in (col - mx)..=(col + mx) {
                                                ...
                                                if best_len < k {
                                                    ...
                                                    while insert_at > 0
                                                        && diff2 < best_dists[insert_at - 1]
                                                    {
                                                        best_dists[insert_at] =
                                                            best_dists[insert_at - 1];
                                                        best_vals[insert_at] =
            ...
            Phase3Op::HighPassMedian => {
                let (_, _, mx, my) = Self::parse_window_sizes(args, 11, 11);
                let mut sig_digits =
                    args.get("sig_digits").and_then(|v| v.as_u64()).unwrap_or(2) as i32;
                ...
                let out_vals: Result<Vec<Vec<f64>>, ToolError> = (0..bands)
                    ...
                    .map(|band_idx| -> Result<Vec<f64>, ToolError> {
                        ...
                        let num_bins = usize::try_from(num_bins_i64).map_err(|_| {
                            ToolError::Execution(
                                "high-pass-median histogram bin count exceeds platform limits"
                                    .to_string(),
                            )
                        })?;
                ...
                if packed_rgb {
                    ...
                    for band_idx in 0..input.bands {
                        ...
                        for (r, row) in rows_buf.iter().enumerate() {
                            out.set_row_slice(band, r as isize, row).map_err(|e| {
                                ToolError::Execution(format!("failed writing row {}: {}", r, e))
                            })?;
                        }
                    }
            ...
            Phase3Op::LaplacianOfGaussians => {
                ...
                let kernel_mean =
                    kernel.iter().map(|(_, _, weight)| *weight).sum::<f64>() / kernel.len() as f64;
