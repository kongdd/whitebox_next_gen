...
impl HydrologicIndexCore {
    ...
    fn relative_stream_power_index_manifest() -> ToolManifest {
        ...
        example_args.insert(
            "output".to_string(),
            json!("relative_stream_power_index.tif"),
        );
        ...
        ToolManifest {
            ...
            params: vec![
                ...
                ToolParamDescriptor {
                    ...
                    description: "Specific catchment area exponent p (default 1.0).".to_string(),
                    required: false,
                },
                ToolParamDescriptor {
    ...
    fn elev_relative_to_watershed_min_max_manifest() -> ToolManifest {
        ...
        example_args.insert(
            "output".to_string(),
            json!("elev_relative_to_watershed_min_max.tif"),
        );
    ...
    fn run_sediment_transport_index(
        ...
        let output_path = parse_optional_output_path(args, "output")?;
        let sca_exponent = args
            .get("sca_exponent")
            .and_then(|v| v.as_f64())
            .unwrap_or(0.4);
        let slope_exponent = args
            .get("slope_exponent")
            .and_then(|v| v.as_f64())
            .unwrap_or(1.3);
        let sca = Self::load_named_raster(args, "sca")?;
    ...
    fn run_elev_relative_to_watershed_min_max(
        ...
        ctx.progress
            .info("running elev_relative_to_watershed_min_max");
        let rows = dem.rows;
        let cols = dem.cols;
        let bands = dem.bands;
        let coalescer = PercentCoalescer::new(1, 99);
...
impl Tool for RelativeStreamPowerIndexTool {
    fn metadata(&self) -> ToolMetadata {
        HydrologicIndexCore::relative_stream_power_index_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        HydrologicIndexCore::relative_stream_power_index_manifest()
    }
...
impl Tool for SedimentTransportIndexTool {
    fn metadata(&self) -> ToolMetadata {
        HydrologicIndexCore::sediment_transport_index_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        HydrologicIndexCore::sediment_transport_index_manifest()
    }
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
        let _ = parse_raster_path_arg(args, "sca")?;
        let _ = parse_raster_path_arg(args, "slope")?;
...
impl Tool for ElevRelativeToWatershedMinMaxTool {
    fn metadata(&self) -> ToolMetadata {
        HydrologicIndexCore::elev_relative_to_watershed_min_max_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        HydrologicIndexCore::elev_relative_to_watershed_min_max_manifest()
    }
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
...
mod tests {
    ...
    fn relative_stream_power_index_matches_expected_formula() {
        ...
        let result = RelativeStreamPowerIndexTool
            .run(&args, &make_ctx())
            .unwrap();
    ...
    fn elev_relative_to_watershed_min_max_matches_zone_percentages() {
        ...
        args.insert(
            "watersheds".to_string(),
            json!(memory_store::make_raster_memory_path(&ws_id)),
        );

        let result = ElevRelativeToWatershedMinMaxTool
            .run(&args, &make_ctx())
            .unwrap();
        let out_path = result.outputs.get("path").unwrap().as_str().unwrap();
        let out_id = memory_store::raster_path_to_id(out_path).unwrap();
