                ...
                band_out
                    ...
                    .for_each(|(r, row_out)| {
                        ...
                        for (c, cell) in row_out.iter_mut().enumerate() {
                            ...
                            let p = projected_scale
                                * (44.0 * (z3 + z23 - z1 - z21)
                                    + 31.0 * (z0 + z20 - z4 - z24 + 2.0 * (z8 + z18 - z6 - z16))
                                    + 17.0 * (z14 - z10 + 4.0 * (z13 - z11))
                                    + 5.0 * (z9 + z19 - z5 - z15));

                            let q = projected_scale
                                * (44.0 * (z5 + z9 - z15 - z19)
                                    + 31.0 * (z20 + z24 - z0 - z4 + 2.0 * (z6 + z8 - z16 - z18))
...
impl Tool for SlopeTool {
    fn metadata(&self) -> ToolMetadata {
        TerrainCore::slope_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        TerrainCore::slope_manifest()
    }
...
impl Tool for AspectTool {
    fn metadata(&self) -> ToolMetadata {
        TerrainCore::aspect_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        TerrainCore::aspect_manifest()
    }
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
        let _ = TerrainCore::parse_input(args)?;
        let _ = parse_optional_output_path(args, "output")?;
...
impl Tool for HillshadeTool {
    fn metadata(&self) -> ToolMetadata {
        TerrainCore::hillshade_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        TerrainCore::hillshade_manifest()
    }
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
        let _ = TerrainCore::parse_input(args)?;
        let _ = parse_optional_output_path(args, "output")?;
...
impl Tool for MultidirectionalHillshadeTool {
    fn metadata(&self) -> ToolMetadata {
        TerrainCore::multidirectional_hillshade_metadata()
    }
    fn manifest(&self) -> ToolManifest {
        TerrainCore::multidirectional_hillshade_manifest()
    }
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
        let _ = TerrainCore::parse_input(args)?;
        let _ = parse_optional_output_path(args, "output")?;
