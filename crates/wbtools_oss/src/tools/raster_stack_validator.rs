    ...
    for (idx, raster) in rasters.iter().enumerate().skip(1) {
        ...
        if !config.allow_no_overlap && crs_compatible(first_crs, &raster.crs) {
            ...
            if !extents_overlap(&first_extent, &extent) {
                ...
                result
                    .errors
                    .push(format!("Raster {} has non-overlapping spatial extent", idx));
                result.is_valid = false;
            }
        }
    }
...
pub fn parse_resample_method(method: &str) -> Option<ResampleMethod> {
    match method.trim().to_ascii_lowercase().as_str() {
        ...
        "stddev" | "standard_deviation" | "standard-deviation" => Some(ResampleMethod::StdDev),
        _ => None,
    }
}
...
pub fn align_and_validate_raster_stack(
    ...
    for (idx, raster) in rasters.iter_mut().enumerate().skip(1) {
        ...
        if raster.rows != reference.rows
            || raster.cols != reference.cols
            || raster.bands != reference.bands
        {
            return Err(format!(
        ...
        if !config.allow_no_overlap {
            ...
            if !extents_overlap(&reference_extent, &extent) {
                return Err(format!("Raster {} has non-overlapping spatial extent", idx));
            }
        }
    }
...
pub fn infer_resampling_method(raster: &Raster) -> &'static str {
    ...
    for (key, value) in &raster.metadata {
        ...
        if key_lower.contains("color_interpretation") || key_lower.contains("photometric") {
            if value_lower.contains("palette")
                || value_lower.contains("index")
                || value_lower.contains("gray")
                || value_lower.contains("palette_index")
            {
                return "nearest";
            }
        }
        ...
        if key_lower.contains("interpretation") {
            if value_lower.contains("class")
                || value_lower.contains("category")
...
mod tests {
    ...
    fn test_extents_overlap() {
        // Simple overlap
        let ext1 = wbraster::Extent {
            x_min: 0.0,
            y_min: 0.0,
            x_max: 10.0,
            y_max: 10.0,
        };
        let ext2 = wbraster::Extent {
            x_min: 5.0,
            y_min: 5.0,
            x_max: 15.0,
            y_max: 15.0,
