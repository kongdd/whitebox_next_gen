...
pub fn raster_tool_param_schemas(tool_id: &str) -> Option<BTreeMap<String, ToolParamSchema>> {
    match tool_id {
        "abs" | "arccos" | "arcosh" | "arcsin" | "arctan" | "arsinh" | "artanh" | "bool_not"
        | "ceil" | "cos" | "cosh" | "exp" | "exp2" | "floor" | "is_nodata" | "ln" | "log10"
        | "log2" | "negate" | "reciprocal" | "round" | "sin" | "sinh" | "sqrt" | "square"
        | "tan" | "tanh" | "to_degrees" | "to_radians" | "truncate" => Some(param_schema_map(&[
        ...
        | "not_equal_to"
        | "power"
        | "subtract" => Some(param_schema_map(&[
        ...
        "assign_projection_raster" => Some(param_schema_map(&[
            ("input", ToolParamSchema::input_raster()),
            ("epsg", ToolParamSchema::scalar_integer()),
        ])),
        "reproject_raster" => Some(param_schema_map(&[
            ("input", ToolParamSchema::input_raster()),
            ("epsg", ToolParamSchema::scalar_integer()),
            ("resample", ToolParamSchema::enum_values(&["nearest", "bilinear", "cubic", "lanczos", "average", "min", "max", "mode", "median", "stddev"])),
            ("output", ToolParamSchema::output_raster()),
        ])),
        ...
        // Kriging and geostatistics tools — output a raster grid
        "ordinary_kriging" | "simple_kriging" | "local_kriging" | "universal_kriging" => {
            Some(param_schema_map(&[
                ("training_points", ToolParamSchema::input_vector_any()),
                ("field", ToolParamSchema::field("training_points", None)),
                ("variogram_json", ToolParamSchema::string()),
                ("template_raster", ToolParamSchema::input_raster()),
                ("output", ToolParamSchema::output_raster()),
        ...
        "spacetime_kriging" => Some(param_schema_map(&[
            ("training_points", ToolParamSchema::input_vector_any()),
            ("field", ToolParamSchema::field("training_points", None)),
            ("time_field", ToolParamSchema::field("training_points", None)),
            ("variogram_json", ToolParamSchema::string()),
            ("template_raster", ToolParamSchema::input_raster()),
            ("output", ToolParamSchema::output_raster()),
        ])),
        "ordinary_cokriging" => Some(param_schema_map(&[
            ("primary_points", ToolParamSchema::input_vector_any()),
            ("primary_field", ToolParamSchema::field("primary_points", None)),
            ("covariate_points", ToolParamSchema::input_vector_any()),
            ("covariate_field", ToolParamSchema::field("covariate_points", None)),
            ("template_raster", ToolParamSchema::input_raster()),
            ("output", ToolParamSchema::output_raster()),
        ])),
        // Spatial statistics raster-output variants
        "getis_ord_gi_star_raster" | "local_morans_i_lisa_raster" => {
            Some(param_schema_map(&[
                ("input", ToolParamSchema::input_vector_any()),
                ("field", ToolParamSchema::field("input", None)),
...
pub use raster_unary_math::{
    RasterAbsTool, RasterArccosTool, RasterArcoshTool, RasterArcsinTool, RasterArctanTool,
    RasterArsinhTool, RasterArtanhTool, RasterBoolNotTool, RasterCeilTool, RasterCosTool,
    RasterCoshTool, RasterDecrementTool, RasterExp2Tool, RasterExpTool, RasterFloorTool,
    RasterIncrementTool, RasterIsNodataTool, RasterLnTool, RasterLog10Tool, RasterLog2Tool,
    RasterNegateTool, RasterReciprocalTool, RasterRoundTool, RasterSinTool, RasterSinhTool,
    RasterSqrtTool, RasterSquareTool, RasterTanTool, RasterTanhTool, RasterToDegTool,
    RasterToRadTool, RasterTruncateTool,
};
