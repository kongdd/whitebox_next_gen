...
Parameters: directions specify azimuths (0-180°) to analyze (common: 0°, 45°, 90°, 135° for cardinal/diagonal); tolerance allows flexibility in direction binning; rose diagram output visualizes anisotropy pattern. Use results to inform universal kriging with anisotropic variogram model."#,
            ...
            params: vec![
                ...
                ToolParamSpec {
                    name: "field",
                    description: "Field with measurement values",
                    required: true,
                },
                ToolParamSpec {
                    name: "directions",
                    description: "Azimuths to analyze (0-180, comma-separated)",
                    required: true,
                },
                ToolParamSpec {
                    name: "tolerance",
                    description: "Direction tolerance (degrees)",
                    required: false,
                },
                ToolParamSpec {
                    name: "max_distance",
                    description: "Maximum lag distance",
                    required: false,
                },
                ToolParamSpec {
                    name: "bin_size",
                    description: "Lag bin size",
                    required: false,
                },
                ToolParamSpec {
                    name: "output_json",
                    description: "Output directional results as JSON",
                    required: true,
                },
                ToolParamSpec {
                    name: "output_html",
                    description: "Optional HTML rose diagram output path",
                    required: false,
                },
            ],
        }
    ...
    fn manifest(&self) -> ToolManifest {
        ...
        defaults.insert(
            ...
            json!("directional_vgram_rose.html"),
        );
        ...
        ToolManifest {
            ...
            tags: vec!["geostatistics".to_string(), "variography".to_string(), "anisotropy".to_string(), "directional".to_string(), "visualization".to_string(), "spatial_statistics".to_string()],
    ...
    fn validate(&self, args: &ToolArgs) -> Result<(), ToolError> {
        ...
        if let Ok(dirs) = directions {
            if dirs.is_empty() {
                return Err(ToolError::Validation(
                    "at least one direction must be specified".to_string(),
                ));
            }
            for dir in &dirs {
            ...
            return Err(ToolError::Validation(
                "directions must be comma-separated numbers".to_string(),
            ));
        }
        ...
        if let Some(path_str) = parse_optional_string_arg(args, "output_html")? {
            if path_str.trim().is_empty() {
                return Err(ToolError::Validation(
                    "output_html path must be non-empty if provided".to_string(),
                ));
            }
        }
        Ok(())
    ...
    fn run(&self, args: &ToolArgs, ctx: &ToolContext) -> Result<ToolRunResult, ToolError> {
        ...
        let output_html = output_html_str
            .as_ref()
        ...
        ctx.progress.info(&format!(
            "Computing variograms for {} samples",
            samples.len()
        ));
        ...
        for (idx, direction) in directions.iter().enumerate() {
            ...
            ctx.progress
                .info(&format!("Computing variogram for direction {}", direction));

            match compute_directional_variogram(
                &samples,
                *direction,
                tolerance,
                max_distance,
                ...
                Err(e) => {
                    ctx.progress.info(&format!(
                        "  Warning: Failed to compute variogram at {}: {}",
                        direction, e
                    ));
                }
            }
        ...
        if vgram_bins.is_empty() {
            return Err(ToolError::Execution(
                "No valid directional variograms computed".to_string(),
            ));
        }
        ...
        // Write output
        ctx.progress
            .info(&format!("Writing results to {}", output_path));
        ...
        if let Some(html_path) = &output_html {
            ctx.progress
                .info("Generating rose diagram visualization...");
            generate_rose_diagram(&vgram_bins, &anisotropy, html_path).map_err(|e| {
                ToolError::Execution(format!("Failed to generate rose diagram: {}", e))
            })?;
        }
...
fn generate_rose_diagram(
    ...
    for (azimuth, range) in vgram_bins
        .iter()
        .map(|v| v.direction_azimuth)
        .zip(ranges.iter())
    {
